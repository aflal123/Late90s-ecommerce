...
import {
  Package, ShoppingBag, Star, Plus, Pencil, Trash2,
  X, Upload, Check, ChevronDown, ToggleLeft, ToggleRight,
  AlertCircle
} from 'lucide-react'
      ...
      <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '2.5rem 3rem' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: 'var(--gold)' }}>{user?.firstName || 'Admin'}</span>
            <br />
            <span style={{ fontSize: '0.6rem', letterSpacing: '0.1em' }}>
          ...
          <div>
            ...
            {productsLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: '4rem', border: '1px dashed var(--border)' }}>
                <p style={{ fontFamily: 'var(--font-inter)', color: 'var(--subtitle)', fontSize: '0.85rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
                ...
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: '60px 1fr 100px 100px 110px 110px 100px',
                  gap: '1rem',
                ...
                {products.map((product, i) => (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      {product.image ? (
                        ...
                        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <span style={{ fontFamily: 'var(--font-bebas)', fontSize: '0.9rem', color: 'var(--border)' }}>L9</span>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Name + sizes */}
                    <div>
                      <div style={{ fontFamily: 'var(--font-bebas)', fontSize: '1.1rem', color: 'var(--text)', letterSpacing: '0.04em', lineHeight: 1 }}>
                        {product.name}
                      </div>
                      <div style={{ fontFamily: 'var(--font-inter)', fontSize: '0.6rem', color: 'var(--muted)', marginTop: '0.2rem', letterSpacing: '0.1em' }}>
                        {product.sizes?.join(' · ')}
                      </div>
                    </div>

                    {/* Price */}
                    <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.8rem', fontWeight: 700, color: 'var(--gold)' }}>
                      {product.price.toLocaleString()}
                    </span>

                    {/* Category */}
                    <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.65rem', color: 'var(--subtitle)', letterSpacing: '0.05em' }}>
                      {product.category}
                    </span>
                    ...
                    <button
                      ...
                      title={product.featured ? 'Remove from featured' : 'Set as featured'}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                    >
                      {product.featured
                        ? <ToggleRight size={22} color="var(--gold)" />
                        : <ToggleLeft size={22} color="var(--border)" />
                      }
                    </button>
                    ...
                    >
                      {product.inStock
                        ? <ToggleRight size={22} color="#6db86d" />
                        : <ToggleLeft size={22} color="var(--border)" />
                      }
                    </button>
          ...
          <div>
            {ordersLoading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: '1fr 1fr 80px 80px 120px 140px',
                  gap: '1rem',
                  padding: '0.6rem 1.25rem',
                  fontFamily: 'var(--font-inter)',
                  fontSize: '0.58rem',
                ...
                {orders.map((order, i) => (
                  ...
                  >
                    ...
                    <div style={{ fontFamily: 'var(--font-inter)', fontSize: '0.8rem', fontWeight: 700, color: 'var(--gold)' }}>
                    ...
                    <select
                      ...
                      style={{
                        backgroundColor: STATUS_COLORS[order.status]?.bg || 'var(--card)',
                        border: `1px solid ${STATUS_COLORS[order.status]?.color || 'var(--border)'}`,
          ...
          <div style={{
            ...
            {reviewsLoading ? (
              Array(3).fill(0).map((_, i) => (
                <div key={i} style={{ height: '180px', backgroundColor: 'var(--card)', border: '1px solid var(--border)', animation: 'pulse 1.5s ease-in-out infinite' }} />
              ))
            ) : reviews.length === 0 ? (
              ...
              >
                ...
                <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.8rem', color: 'var(--subtitle)', lineHeight: 1.7, marginBottom: '1rem' }}>
                  {review.comment}
                </p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                  <div style={{
                    ...
                    backgroundColor: 'var(--gold)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: 'var(--font-bebas)', fontSize: '0.9rem', color: '#000',
                  }}>
                    {review.customerName[0]}
                  </div>
                  <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.75rem', fontWeight: 600, color: 'var(--text)' }}>
                    {review.customerName}
                  </span>
                </div>
              </motion.div>
          ...
          <>
            ...
            >
              ...
              <div style={{
                ...
                <button onClick={closePanel} style={{ background: 'none', border: 'none', color: 'var(--subtitle)', cursor: 'pointer' }}>
                  <X size={20} />
                </button>
              </div>
              ...
              <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', flex: 1 }}>
                  ...
                  >
                    {imagePreview ? (
                      ...
                      <div style={{ textAlign: 'center' }}>
                        ...
                        <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.7rem', color: 'var(--muted)', marginTop: '0.5rem' }}>
                    ...
                    {ALL_SIZES.map(size => (
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${form.sizes.includes(size) ? 'var(--gold)' : 'var(--border)'}`,
                          backgroundColor: form.sizes.includes(size) ? 'var(--gold)' : 'transparent',
                          color: form.sizes.includes(size) ? '#000' : 'var(--subtitle)',
                          fontFamily: 'var(--font-inter)',
                          fontSize: '0.65rem',
                          fontWeight: 600,
                          cursor: 'pointer',
                ...
                <div style={{ display: 'flex', gap: '1.5rem' }}>
                  {[
                    ...
                    { key: 'featured', label: 'Featured Drop' },
                  ].map(toggle => (
                    <button
                      key={toggle.key}
                      type="button"
                      ...
                      style={{
                        ...
                        border: 'none',
                        cursor: 'pointer',
                        color: form[toggle.key] ? 'var(--gold)' : 'var(--muted)',
                        fontFamily: 'var(--font-inter)',
                        fontSize: '0.7rem',
                        fontWeight: 600,
                        letterSpacing: '0.1em',
              ...
              <div style={{
                ...
                >
                  {saving ? (uploading ? 'Uploading Image...' : 'Saving...') : (editingProduct ? 'Save Changes' : 'Add Product')}
                </motion.button>
              </div>
            </motion.div>
          </>
      ...
      <AnimatePresence>
        {deleteConfirm && (
          <>
            ...
            >
              <div style={{ fontFamily: 'var(--font-bebas)', fontSize: '1.8rem', color: '#dc5050', marginBottom: '0.5rem' }}>
                Delete Product?
              </div>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button
                  onClick={() => setDeleteConfirm(null)}
                  style={{ flex: 1, padding: '0.75rem', backgroundColor: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--font-inter)', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}
                >
                  Cancel
                </button>
                <button
                  onClick={() => handleDelete(deleteConfirm)}
                  style={{ flex: 1, padding: '0.75rem', backgroundColor: '#dc5050', border: 'none', color: '#fff', fontFamily: 'var(--font-inter)', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer' }}
                >
                  Delete
                </button>
              </div>
